...
#define G_LOG_DOMAIN "Elgato Stream Deck Original"
...
struct _ElgatoStreamDeckOriginal
{
  ElgatoStreamDeckGen1 parent_instance;
};

G_DEFINE_FINAL_TYPE (ElgatoStreamDeckOriginal, elgato_stream_deck_original, ELGATO_TYPE_STREAM_DECK_GEN1)
...
swap_button_index_original (ElgatoStreamDeck *stream_deck,
                            uint8_t           button_index)
{
  BsDeviceRegion *button_grid = bs_device_get_region (BS_DEVICE (stream_deck), "main-button-grid");
  int n_columns = bs_button_grid_get_grid_columns (BS_BUTTON_GRID (button_grid));
  int column = button_index % n_columns;
  int actual_index = ((int) button_index - column) + (n_columns - 1 - column);
...
static gboolean
elgato_stream_deck_original_read_state (ElgatoStreamDeck *stream_deck)
{
  ...
  for (uint8_t i = 0; i < n_buttons; i++)
    {
      ...
      position = swap_button_index_original (stream_deck, i);
      button = find_button_at_region (stream_deck, "main-button-grid", position);
...
static gboolean
elgato_stream_deck_original_set_button_texture (ElgatoStreamDeck  *stream_deck,
                                                BsButton          *button,
                                                GdkTexture        *texture,
                                                GError           **error)
{
  ...
  payload[5] = swap_button_index_original (stream_deck, button_index) + 1;
  payload[6] = 0;
  payload[7] = 0;
  payload[8] = 0;
  payload[9] = 0;
...
static GListModel *
elgato_stream_deck_original_create_layout (BsDevice *device)
{
  g_autoptr (BsDeviceLayoutBuilder) layout_builder = NULL;

  g_assert (ELGATO_IS_STREAM_DECK_ORIGINAL (device));
...
static const char *
elgato_stream_deck_original_get_name (BsDevice *device)
{
   ...
   */
  return _("Elgato Stream Deck");
}


static void
elgato_stream_deck_original_class_init (ElgatoStreamDeckOriginalClass *klass)
{
  BsDeviceClass *device_class = BS_DEVICE_CLASS (klass);
  ElgatoStreamDeckClass *stream_deck_class = ELGATO_STREAM_DECK_CLASS (klass);

  device_class->create_layout = elgato_stream_deck_original_create_layout;
  device_class->get_name = elgato_stream_deck_original_get_name;

  stream_deck_class->read_state = elgato_stream_deck_original_read_state;
  stream_deck_class->set_button_texture = elgato_stream_deck_original_set_button_texture;
}

static void
elgato_stream_deck_original_init (ElgatoStreamDeckOriginal *self)
{
}
