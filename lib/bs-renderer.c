...
static GBytes *
convert_texture (GdkTexture  *texture,
                 const char  *format_mimetype,
                 GError     **out_error)
{
  ...
  creator = gly_creator_new (format_mimetype, out_error);
  if (!creator)
    return NULL;

  gly_creator_set_encoding_quality (creator, 97);
  ...
  encoded_image = gly_creator_create (creator, out_error);
  if (!encoded_image)
    return NULL;
                             ...
                             GError     **error)
{
  ...
  switch (self->image_info.format)
    ...
    case BS_IMAGE_FORMAT_BMP:
      return convert_texture (texture, "image/bmp", error);
