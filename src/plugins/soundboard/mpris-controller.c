...
mpris_controller_dispose (GObject *object)
{
  ...
  g_clear_handle_id (&self->namespace_watcher_id, bus_unwatch_namespace);
                                   ...
                                   MprisController *self)
{
  ...
  g_object_notify (G_OBJECT (self), "has-player");
                               ...
                               GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_IS_PLAYING:
      g_value_set_boolean (value, mpris_controller_get_is_playing (self));
      break;

    case PROP_HAS_ACTIVE_PLAYER:
      g_value_set_boolean (value, mpris_controller_get_has_active_player (self));
      break;

    case PROP_HAS_PLAYER:
      g_value_set_boolean (value, mpris_controller_get_has_player (self));
      break;
...
mpris_controller_class_init (MprisControllerClass *klass)
{
  ...
  g_object_class_install_property (object_class,
                                   PROP_HAS_PLAYER,
                                   g_param_spec_boolean ("has-player",
                                                         NULL,
                                                         NULL,
                                                         FALSE,
                                                         G_PARAM_READABLE));
...
gboolean
mpris_controller_key (MprisController *self,
                      const gchar     *key)
{
  ...
  g_debug ("Calling %s over D-Bus to mpris client %s",
           key,
           g_dbus_proxy_get_name (self->mpris_client_proxy));
...
gboolean
mpris_controller_seek (MprisController *self,
                       gint64           offset)
{
  g_return_val_if_fail (MPRIS_IS_CONTROLLER (self), FALSE);
  ...
  g_debug ("Calling Seek over D-Bus to mpris client %s",
           g_dbus_proxy_get_name (self->mpris_client_proxy));

  g_dbus_proxy_call (self->mpris_client_proxy,
                     "Seek",
                     g_variant_new ("(x)", offset, NULL),
...
gboolean
mpris_controller_toggle (MprisController *self,
...
gboolean
mpris_controller_get_has_player (MprisController *self)
{
  g_return_val_if_fail (MPRIS_IS_CONTROLLER (self), FALSE);

  return self->mpris_client_proxy != NULL || self->other_proxies != NULL;
}

gboolean
mpris_controller_get_is_playing (MprisController *self)
{
  g_return_val_if_fail (MPRIS_IS_CONTROLLER (self), FALSE);

  return self->mpris_client_proxy && mpris_client_is_playing (self->mpris_client_proxy);
}
